...
class ComplaintsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create complaint" do
    assert_difference('Complaint.count') do
      post complaints_url, params: { complaint: { invoice: @complaint.invoice, issue_date: @complaint.issue_date, lot: @complaint.lot, notes: @complaint.notes, order: @complaint.order, status: @complaint.status, user: @complaint.user, with_vendor: @complaint.with_vendor } }
    end
  ...
  test "should update complaint" do
    patch complaint_url(@complaint), params: { complaint: { invoice: @complaint.invoice, issue_date: @complaint.issue_date, lot: @complaint.lot, notes: @complaint.notes, order: @complaint.order, status: @complaint.status, user: @complaint.user, with_vendor: @complaint.with_vendor } }
